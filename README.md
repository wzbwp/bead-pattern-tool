...
默认使用 `最多 8 色`，以减少图片抗锯齿和压缩噪声产生的近似色。
...
默认的 `经典还原（原版效果）` 完整使用历史版本的 9 x 9 区域采样和颜色权重，并跳过后续颜色修正。要复现同一张历史图纸，必须使用同一原图、相同宽高和相同颜色上限；例如示例旧图使用 `80 x 106` 和 `最多 8 色`。`平衡还原` 会额外保护连续细线，可在经典模式无法保留细线时手动切换。
